...
const Trend = ({result, isLoading}) => {

   
  const getAllTrend = result?.data.length > 0 && result?.data.filter((item) => item.items_category === "Trend" )
  ...
  return (
   <div className='pt-10'>
        <Slider {...settings}>
        {!isLoading && getAllTrend.map((item, key)=>(
