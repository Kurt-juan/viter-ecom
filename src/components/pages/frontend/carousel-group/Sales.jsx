...
const Sales = ({result, isLoading}) => {

   
  const getAllSales = result?.data.length > 0 && result?.data.filter((item) => item.items_category === "New Arrival" )
  ...
  return (
   <div className='pt-10'>
        <Slider {...settings}>
        {!isLoading && getAllSales.map((item, key)=>(
