...
import ItemsModalAdd from './ItemsModalAdd';
...
const ItemsTable = () => {
  ...
  const {
    ...
    queryKey: ["items", onSearch, store.isSearch],
    queryFn: async ({ pageParam = 1 }) =>
      await queryDataInfinite(
        `/v1/items/search`, // search endpoint
        `/v1/items/page/${pageParam}`, // list endpoint
  ...
  return (
    ...
    <div className="flex justify-between items-center">
      <SearchBar  
        ...
        onSearch={onSearch}
      />
      <button className="btn btn-black " onClick={handleAdd} >
      ...
      <React.Fragment key={key}>
        {page.data.map((item, key) => {
         ...
         <tr key={key}>
          <td>{count} </td>
          <td>
              <Pill isActive={item.items_is_active}/>
          </td>
...
<ModalConfirm
    queryKey ="items"
    mysqlApiArchive ={`/v1/items/active/${id}`}
    active={isActive}
    
/>}

{store.isDelete && <ModalDelete 
    mysqlApiDelete ={`/v1/items/${id}`}   
    queryKey ="items"
...
{store.isAdd && <ItemsModalAdd itemEdit={itemEdit}/>}
  </>
  )
}
